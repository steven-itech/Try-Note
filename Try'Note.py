...
def submit():
    ...
    if not all([subject_entry, softwares_entry, techniques_entry, conclusion_entry, pentester_entry]):
        ...
        messagebox.showwarning(title="Try'Note : ", message=warning)
    ...
    if not all(any(c.isalpha() for c in entry) and any(c.isspace() for c in entry) for entry in [softwares_entry, techniques_entry, conclusion_entry, pentester_entry]):
        ...
        pdf.image("hacking.png", x=10, y=504, w=466)
        ...
        tts(confirmation)
        messagebox.showinfo(title="Try'Note :", message=confirmation)

if __name__ == "__main__":

    ctypes.windll.kernel32.SetConsoleTitleW("Try'Note :")
    
    window = tk.Tk()
    window.title("Try'Note :")
    window.iconbitmap("thm.ico")
